	...
	while(greedy._vector.empty()!=1){
		...
		if( inloop.x!=1 && !visited[inloop.y][inloop.x-1]){
			...
			if(arr_maze[inloop.y][inloop.x-1]==0){
				...
				up.direction = 1;						//set direction of node

				up.hfunc = abs(up.x-a)+abs(up.y-b) + up.pathcost;


				if(inloop.direction == 1)				//if the direction is the same add 1 to the pathcost to reflect
		...
		if( inloop.x!=(i-1) && !visited[inloop.y][inloop.x+1]){
			...
			if(arr_maze[inloop.y][inloop.x+1]==0){
				...
				if(inloop.direction == 3)				//if the direction is the same add 1 to the pathcost to reflect
					up.pathcost = up.pathcost + 1;

				up.direction = 3;		//set direction of node
		...
		if( inloop.y!=1 && !visited[inloop.y-1][inloop.x]){
			...
			if(arr_maze[inloop.y-1][inloop.x]==0){
				...
				if(inloop.direction == 4)				//if the direction is the same add 1 to the pathcost to reflect
					up.pathcost = up.pathcost + 1;

				up.direction = 4;			//set direction of node
		...
		if( inloop.y!=(j-2) && !visited[inloop.y+1][inloop.x]){
			...
			if(arr_maze[inloop.y+1][inloop.x]==0){
				...
				if(inloop.direction == 2)			//if the direction is the same add 1 to the pathcost to reflect
					up.pathcost = up.pathcost + 1;

				up.direction = 2;			//set direction of node
